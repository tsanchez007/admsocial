...
// POST /api/accounts — agregar cuenta manualmente (para pruebas)
router.post('/', (req, res) => {
    ...
    if (existing) {
        db.prepare(`UPDATE accounts SET access_token = ?, page_id = ?, ig_account_id = ?, status = 'connected' WHERE id = ?`)
          .run(access_token, page_id || null, ig_account_id || null, existing.id);
        return res.json({ success: true, message: 'Cuenta actualizada' });
    }

    db.prepare(`
        INSERT INTO accounts (platform, username, access_token, page_id, ig_account_id)
        VALUES (?, ?, ?, ?, ?)
...
router.delete('/:id', (req, res) => {
    db.prepare(`UPDATE accounts SET status = 'disconnected' WHERE id = ?`).run(req.params.id);
